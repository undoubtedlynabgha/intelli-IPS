...
Write-Host "   INTELLI IPS â€” FULL BUILD PIPELINE" -ForegroundColor Cyan
Write-Host "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•" -ForegroundColor Cyan
Write-Host ""

# â”€â”€â”€ 1. Verify .env.local has the API key â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
Write-Host "[ 1/5 ] Checking .env.local for GROQ_API_KEY..." -ForegroundColor Yellow

$envFile = Join-Path $PSScriptRoot ".env.local"
if (-not (Test-Path $envFile)) {
    Write-Host "  âš   .env.local not found. Creating a template..." -ForegroundColor Red
...
if ($envContent -notmatch "GROQ_API_KEY\s*=\s*[A-Za-z0-9_\-]{10,}") {
    Write-Host "  âš   GROQ_API_KEY appears to be missing or invalid in .env.local" -ForegroundColor Red
    ...
    Write-Host "  âœ”  GROQ_API_KEY found in .env.local" -ForegroundColor Green
}

# â”€â”€â”€ 2. Build Python backend with PyInstaller â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (-not $SkipBackend) {
    Write-Host ""
    ...
    if (-not (Test-Path $exePath)) {
        Write-Host "  âœ–  PyInstaller FAILED â€” ips_backend.exe not found at:" -ForegroundColor Red
        Write-Host "     $exePath" -ForegroundColor Red
        exit 1
    }

    $exeSize = [math]::Round((Get-Item $exePath).Length / 1MB, 1)
    Write-Host "  [OK] Backend built successfully ($exeSize MB): $exePath" -ForegroundColor Green
    Set-Location $PSScriptRoot
} else {
    Write-Host "[ 2/5 ] Skipping backend build (-SkipBackend flag)" -ForegroundColor DarkGray
}

# â”€â”€â”€ 3. Verify backend exe exists â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
Write-Host ""
...
if (-not (Test-Path $backendExe)) {
    Write-Host "  âœ–  backend\dist\ips_backend.exe not found!" -ForegroundColor Red
    Write-Host "     Run without -SkipBackend to build it first." -ForegroundColor Red
    exit 1
}
Write-Host "  âœ”  Backend exe verified." -ForegroundColor Green

# â”€â”€â”€ 4. Build the React/Vite frontend â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (-not $SkipFrontend) {
    Write-Host ""
    ...
    Write-Host "  âœ”  Frontend built into dist\" -ForegroundColor Green
} else {
    Write-Host "[ 4/5 ] Skipping frontend build (-SkipFrontend flag)" -ForegroundColor DarkGray
}

# â”€â”€â”€ 5. Package with Electron Builder (NSIS installer) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
Write-Host ""
...
if ($installers) {
    ...
    foreach ($ins in $installers) {
        $sizeMB = [math]::Round($ins.Length / 1MB, 1)
        Write-Host "    >> $($ins.Name)  ($sizeMB MB)" -ForegroundColor Cyan
    }
} else {
